...
invoke `railyard:model-routing` with exact contract
`railyard/model-routing/v1` and consume its immutable snapshot (policy
...
The controller cannot switch its own session model; children carry the
...
`railyard:model-routing` on every work-starting create, message, or
...
prerequisites — delegate inventory and user-approved reconciliation to
Roundhouse; never mutate hosts directly. Dispatch only after every assigned
