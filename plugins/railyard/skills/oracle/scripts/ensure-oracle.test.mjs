...
const NPM_PACKAGE = "@steipete/oracle@0.20.3";
...
function createFixture(initialState, npmState = "missing") {
  ...
  if (["current", "multi-keg", "prefix-failure", "bad-executable-version"].includes(initialState)) {
    writeOracle(
      ...
      initialState === "bad-executable-version" ? "0.9.9" : "0.20.3",
    );
  }
  if (initialState === "version-failure") {
    writeOracle(path.join(fixture.prefix, "bin", "oracle"), "0.20.3", true);
  }
  if (["current", "stale"].includes(npmState)) {
    writeOracle(path.join(fixture.npmPrefix, "bin", "oracle"), npmState === "stale" ? "0.9.9" : "0.20.3");
  }
...
write_oracle() {
  ...
  printf '%s\\n' '#!/bin/sh' 'printf "%s\\n" "oracle 0.20.3"' > "$ORACLE_TEST_PREFIX/bin/oracle"
...
case "$1" in
  ...
  install)
    ...
    ;;
  upgrade)
    if [ "$2" != "--formula" ] || [ "$3" != "--minimum-version" ] || [ "$4" != "0.20.3" ] || [ "$5" != "--no-ask" ] || [ "$6" != "${FORMULA}" ] || [ "$#" -ne 6 ]; then
...
test("invalid explicit overrides fail validation without Homebrew", () => {
  ...
  const beforeSupportedOracle = path.join(fixture.directory, "before-supported-oracle");
  ...
  try {
    ...
    writeOracle(beforeSupportedOracle, "0.20.2");
    writeOracle(prereleaseOracle, "0.20.3-beta.1");
    fs.writeFileSync(nonExecutable, "#!/bin/sh\nexit 0\n", { mode: 0o644 });

    for (const oracleBin of ["", "oracle", fixture.directory, nonExecutable, oldOracle, beforeSupportedOracle, prereleaseOracle]) {
...
test("a 0.9.x formula upgrades numerically with the narrow Homebrew environment", () => {
  ...
  try {
    ...
    assert.equal(result.stdout.trim(), expected);
    assert.deepEqual(brewMutations(fixture), [
      `upgrade --formula --minimum-version 0.20.3 --no-ask ${FORMULA}|cleanup=1|dependents=1`,
    ]);
    assert.deepEqual(npmMutations(fixture), []);
    assert.equal(fs.readFileSync(fixture.state, "utf8"), "current\n");
    assertOracleStateIsPreserved(fixture);
