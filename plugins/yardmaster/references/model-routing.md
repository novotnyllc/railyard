...
`yardmaster:model-routing` is the single public decision point for model,
effort, budget-admission, and dispatch-transport policy. Its exact wire version
is `yardmaster/model-routing/v1`.
...
also carries `yardmaster/r28-route-disclosure/v1`. It gives requested,
...
`yardmaster/action-receipt/v1` schema. It records the stable action ID and
...
The policy path order is absolute `YARDMASTER_MODEL_POLICY_PATH`, then
the platform user config path. The state path order is absolute
`YARDMASTER_MODEL_STATE_PATH`, then the POSIX user state path. Defaults
...
```bash
node --test plugins/yardmaster/scripts/model-routing.test.mjs
```
