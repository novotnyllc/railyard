...
const IMPORTER_ID = "yardmaster-adapter-receipt-importer-v1";
...
function refreshAttestor({ observedModel, capabilities = [], authState = "authenticated", fallbackSetDigest, expiresAt = "2026-08-04T12:30:00.000Z" } = {}) {
  return (record) => {
    ...
    const result = {
      attestorId: "yardmaster-host-attestor-v1",
...
function trustedTaskAuthorityAttestor(now = NOW) {
  return ({ authority }) => {
    ...
    return {
      attestorId: "yardmaster-task-authority-attestor-v1",
...
function r52Readiness() {
  return {
    schema: "yardmaster/r52-readiness/v1",
...
test("catalogs, privacy, and closed CE seams cannot widen routing authority", () => {
  ...
  const plan = handleRequest(request("resolve", {
    ...
    ceSeam: { id: "ce-plan.execution", skill: "ce-plan", artifact: { schema: "yardmaster/ce-plan-execution-input/v1", digest: DIGEST_A } },
  }), { now: NOW });
  ...
  const incompatible = handleRequest(request("resolve", {
    callerKind: "compound-engineering",
    ceSeam: { id: "ce-code-review.execution", skill: "ce-code-review", artifact: { schema: "yardmaster/ce-code-review-findings/v1", digest: DIGEST_A } },
  }), { now: NOW });
...
test("R28 decision, fallback, and settlement disclosures use explicit provenance without task content", () => {
  const terra = handleRequest(request("resolve"), {
    ...
    trustedRuntimeAttestor: () => ({
      attestorId: "yardmaster-runtime-attestor-v1",
      attestationDigest: DIGEST_A,
  ...
  assert.equal(terra.response.decision.fallbackReceipt.schema, "yardmaster/r28-route-disclosure/v1");
...
test("visible bridge acknowledgement and activation bind the exact fixed task identity", () => {
  ...
  const transportAttestor = () => ({
    attestorId: "yardmaster-transport-attestor-v1",
    attestationDigest: DIGEST_A,
...
test("the public CLI accepts only a fixed Oracle receipt reference and settles an adapter-emitted private artifact", () => {
  ...
  try {
    ...
    const configDirectory = path.join(home, ".config", "yardmaster");
    const stateDirectory = path.join(home, ".local", "state", "yardmaster");
    ...
    const root = path.join(home, ".local", "state", "yardmaster", "oracle-route");
    ...
    assert.equal(settled.disclosure.schema, "yardmaster/r28-route-disclosure/v1");
  ...
  try {
    ...
    const nativeSettled = publicCli({
      ...
      receipt: { appToolEvidence: {
        schema: "yardmaster/app-tool-evidence/v1",
...
test("protected inspect-claim ignores caller path and XDG overrides", () => {
  ...
  try {
    ...
    const inspected = runCli(request("inspect-claim", {
      ...
      cwd: process.cwd(),
      env: {
        YARDMASTER_MODEL_POLICY_PATH: configPath,
        YARDMASTER_MODEL_STATE_PATH: statePath,
...
test("state paths fail closed for a selected missing policy and the paired fast-path fixture proves no I/O", () => {
  ...
  try {
    ...
    assert.equal(resolvePaths({ home: directory, cwd: process.cwd(), env: { YARDMASTER_MODEL_POLICY_PATH: selected } }).ok, true);
    assert.equal(runCli(request("validate"), { cwd: process.cwd(), env: { YARDMASTER_MODEL_POLICY_PATH: selected, YARDMASTER_MODEL_STATE_PATH: path.join(directory, "state.json") } }).reason, "selected_policy_missing");
  } finally {
    fs.rmSync(directory, { recursive: true, force: true });
  }
