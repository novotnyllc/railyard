...
export function createMutationLock({
  fsApi = fs,
  uid = callerUid(),
  lockPath = path.join(os.tmpdir(), `yardmaster-cleanup-codex-${uid}.lock`),
...
function strictLauncherPath({ explicit, env, fsApi }) {
  ...
  if (env.YARDMASTER_CODEX_BIN) return env.YARDMASTER_CODEX_BIN;
  const candidates = [];
...
function hookStateDirectory(env) {
  if (env.XDG_STATE_HOME) {
    ...
    return path.join(env.XDG_STATE_HOME, "yardmaster", "cleanup-codex");
  }
  const home = env.HOME || os.homedir();
  if (!path.isAbsolute(home)) throw new Error("hook-state-path-invalid");
  return path.join(home, "Library", "Application Support", "yardmaster", "cleanup-codex");
}
...
export function inspectHook({
  ...
  if (env.YARDMASTER_CLEANUP_CODEX_HOOK_DISABLED === "1") {
...
function usage() {
  return [
    ...
    "Unmanaged receipt and recycle also require --unmanaged and --launcher PATH (or YARDMASTER_CODEX_BIN).",
...
export function runCli(argv = process.argv.slice(2), {
  ...
  if (parsed.action === "recycle") {
    const launcher = parsed.unmanaged
      ...
      : null;
    const attestorPath = parsed.nofileAttestor ?? env.YARDMASTER_NOFILE_ATTESTOR ?? null;
